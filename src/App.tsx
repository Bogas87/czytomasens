...
const STORAGE_KEY = "ctms_premium_front_v11";

const CONSENTS = [
  "Rozumiem, że CzyToMaSens jest narzędziem analitycznym, a nie poradą psychologiczną, terapeutyczną ani medyczną.",
  "Rozumiem, że wynik analizy opiera się wyłącznie na moich odpowiedziach i nie stanowi diagnozy ani oceny drugiej osoby.",
  "Wyrażam zgodę na przetwarzanie moich danych w celu przygotowania raportu, zgodnie z Polityką prywatności.",
  "Rozumiem, że pełny raport jest przygotowywany bezpośrednio po płatności i tracę prawo do odstąpienia od umowy w momencie rozpoczęcia jego przygotowania.",
];

const LEGAL_CONTENT: Record<Exclude<LegalKey, null>, { title: string; body: string }> = {
  regulamin: {
    title: "Regulamin",
    body: `CzyToMaSens jest narzędziem analitycznym i refleksyjnym — nie zastępuje psychoterapii, doradztwa psychologicznego ani prawnego. Raporty mają charakter informacyjny i opierają się wyłącznie na odpowiedziach użytkownika.\n\nPłatność: dostęp do pełnego raportu jest odpłatny. Płatność realizowana jest przez Stripe. Po skutecznej płatności przygotowywany jest indywidualny raport.\n\nTreść cyfrowa: raport stanowi treść cyfrową dostarczaną bezpośrednio po zakupie. Zgodnie z art. 38 pkt 13 ustawy o prawach konsumenta, prawo do odstąpienia nie przysługuje po rozpoczęciu realizacji treści cyfrowej, na co użytkownik wyraża zgodę przed zakupem.\n\nReklamacje: kontakt@czytomasens.pl — rozpatrywane w 14 dni roboczych.\n\nOdpowiedzialność: usługodawca nie ponosi odpowiedzialności za decyzje podjęte na podstawie treści raportu.`,
  },
  prywatnosc: {
    title: "Polityka prywatności",
    body: `Administrator: właściciel serwisu CzyToMaSens, kontakt@czytomasens.pl.\n\nZakres danych: adres e-mail, treści odpowiedzi analitycznych, dane transakcyjne (Stripe), dane techniczne (IP, user-agent).\n\nCele: realizacja usługi, obsługa płatności, kontakt, bezpieczeństwo systemu.\n\nOdbiorcy: Stripe Inc. (płatności), dostawca hostingu, Resend (e-mail). Dane nie są sprzedawane.\n\nOkres przechowywania: dane analityczne — 90 dni, dokumentacja transakcyjna — 5 lat.\n\nPrawa: dostęp, sprostowanie, usunięcie, ograniczenie, przenoszenie, sprzeciw, cofnięcie zgody, skarga do UODO. Kontakt: kontakt@czytomasens.pl.`,
  },
  rodo: {
...
const ENTRY_CONFIGS: EntryConfig[] = [
  ...
  },
  {
    ...
    questions: [
      ...
      { id: "u5", lead: "Plany to deklaracje intencji. Realizacja to prawda.", text: "Czy ta osoba pojawia się w Twoich planach i Ty w jej — konkretnych, nie mglistych?", options: [{ id: "a", label: "Nie. Plany to u nas abstrakcja.", score: 3 }, { id: "b", label: "Rzadko. I zwykle to ja proponuję.", score: 2 }, { id: "c", label: "Czasem. Ale nie jest to naturalne.", score: 1 }, { id: "d", label: "Tak. Jesteśmy w swoich planach.", score: 0 }] },
  ...
  },
  {
    ...
    questions: [
      ...
      { id: "s6", lead: "Wspólne chwile mówią więcej niż deklaracje.", text: "Czy jest coś co robicie razem i oboje naprawdę tego chcecie — coś co nie jest obowiązkiem?", options: [{ id: "a", label: "Trudno mi cokolwiek wymienić.", score: 3 }, { id: "b", label: "Było — ale dawno. Teraz głównie rutyna.", score: 2 }, { id: "c", label: "Jedno dwie rzeczy — ale to za mało.", score: 1 }, { id: "d", label: "Tak. Mamy wspólne rzeczy które lubimy.", score: 0 }] },
...
function hasLowQualityContent(text: string): boolean {
  ...
  const jokePatterns = [
    /\b(test|testing|pr[oó]ba|haha|hehe|lol|xd|żart|jaja|bek[aę]|dupa|g[łl]upoty|asdf|qwerty)\b/i,
    /(.)\1{5,}/,
    /^[a-ząćęłńóśźż]{1,3}(\s+[a-ząćęłńóśźż]{1,3}){3,}$/i,
  ];
  if (jokePatterns.some((re) => re.test(value))) return true;
  if (letters < 14 || spaces < 2 || uniqueChars < 8) return true;
  return false;
}

function getPatternName(pathKey: EntryKey | null, preview: Preview): string {
  if (preview.tone === "green") return "Układ z realnym gruntem";
  if (pathKey === "uncertain") return "Relacja zawieszonej decyzji";
  if (pathKey === "loop") return "Układ napięcie–ulga";
  if (pathKey === "returning") return "Powrót do historii, która miała się domknąć";
  if (pathKey === "triangle") return "Trzecia osoba jako lustro braków";
  if (pathKey === "betrayal") return preview.tone === "red" ? "Zaufanie pod stałym nadzorem" : "Odbudowa pod warunkiem spójności";
  if (pathKey === "stagnation") return "Spokój, który może być wygaszeniem";
  return "Wzorzec wymagający nazwania";
}

function getRememberLine(pathKey: EntryKey | null, preview: Preview): string {
  if (preview.tone === "green") return "Nie każdy trudny moment oznacza koniec. Ale każdy wymaga uczciwego nazwania tego, co naprawdę działa.";
  if (pathKey === "uncertain") return "Niepewność, która trwa za długo, często nie jest brakiem odpowiedzi. Jest odpowiedzią.";
  if (pathKey === "loop") return "Jeśli relacja działa najmocniej wtedy, gdy boisz się ją stracić, to może trzymać ją napięcie, nie spokój.";
  if (pathKey === "returning") return "Czasem nie wracasz do człowieka, tylko do wersji historii, która miała się lepiej skończyć.";
  if (pathKey === "triangle") return "Trzecia osoba rzadko tworzy problem od zera. Częściej pokazuje brak, który już był.";
  return "To, że coś boli, nie znaczy jeszcze, że jest bez sensu. Ale ból nie może być jedynym dowodem wartości.";
}

function getRelationAxes(preview: Preview) {
  const clarity = safeNumber(100 - Math.round(preview.asymmetry * 0.72), 8, 92);
  const reciprocity = safeNumber(100 - Math.round(preview.asymmetry * 0.58), 10, 94);
  const repetition = safeNumber(Math.round((preview.tension + preview.asymmetry) / 2), 10, 96);
  const selfDeception = safeNumber(Math.round((preview.tension * 0.55) + (preview.asymmetry * 0.35)), 8, 94);
  return [
    { label: "Napięcie", value: preview.tension, hint: "Ile emocjonalnego obciążenia niesie ta sytuacja." },
    { label: "Jasność", value: clarity, hint: "Na ile wiadomo, czym naprawdę jest ta relacja." },
    { label: "Wzajemność", value: reciprocity, hint: "Czy wysiłek i inicjatywa rozkładają się podobnie po obu stronach." },
    { label: "Powtarzalność", value: repetition, hint: "Czy problem wygląda jak jednorazowy kryzys, czy jak wzorzec." },
    { label: "Zmiana", value: preview.change, hint: "Czy są realne podstawy, by sytuacja mogła się poprawić." },
    { label: "Samooszukiwanie", value: selfDeception, hint: "Ryzyko, że nadzieja przykrywa fakty." },
  ];
}
...
async function fetchPaidReport(token: string): Promise<FullReport> {
  ...
  throw new Error("Raport jest jeszcze przygotowywany. Sprawdź skrzynkę — wyślemy Ci link, gdy będzie gotowy.");
}
...
async function fetchPreviewFromAPI(token: string, path: EntryConfig, answers: AnswerMap, openText: string): Promise<Preview> {
  ...
  try {
    ...
    if (data?.ok && data?.preview) {
      ...
      return { chance: clamp(chance, 5, 95), tension: clamp(tension, 5, 97), asymmetry: clamp(asymmetry, 5, 97), change: clamp(change, 5, 90), tone: chance <= 30 ? "red" : chance <= 60 ? "yellow" : "green", badge: p.subheadline || "Analiza relacji", headline: p.headline || "Wynik gotowy.", truth: p.previewLine || "", mirror: p.sections?.[0]?.text || "", summary: p.sections?.[1]?.text || p.sections?.[0]?.text || "", paidTease: "To tylko fragment. Pełny raport pokazuje pełną mapę sytuacji: wzorzec, ryzyka, potencjał zmiany i punkt decyzyjny." } as Preview;
    }
...
const PROCESSING_MESSAGES = [
  "Sprawdzam wzorzec relacyjny...",
  "Wyłapuję mechanizmy obronne...",
  "Układam dynamikę zaangażowania...",
  "Szukam miejsc, w których odpowiedzi się rozjeżdżają...",
  "Porządkuję profil przywiązania...",
  "Przygotowuję sekcję lustra...",
  "Domykam raport...",
];
  ...
  return (
    <div className="loading-wrap">
      <Glass className="loading-panel">
        ...
        <h2 style={{ marginBottom: "12px", fontSize: "clamp(20px,4vw,26px)" }}>Raport jest przygotowywany</h2>
        <div className="processing-message">{PROCESSING_MESSAGES[msgIndex]}{".".repeat(dots)}</div>
        <p style={{ marginTop: "24px", fontSize: "14px", color: "var(--muted)", lineHeight: 1.6 }}>Raport powstaje wyłącznie na podstawie Twoich odpowiedzi.<br />Nie zamykaj karty — to zajmie zwykle 1–2 minuty.</p>
...
export default function App() {
  ...
  const sendInterviewAnswer = async () => {
    ...
    if (hasLowQualityContent(interviewAnswer)) { setError("Odpowiedź wygląda na testową albo zbyt przypadkową. Napisz konkretnie, co się wydarzyło lub co realnie czujesz — inaczej wynik będzie bezwartościowy."); return; }
    ...
    } catch (e: any) { setError(e?.message || "Nie udało się przejść dalej."); }
    finally { setInterviewBusy(false); }
  };
  ...
  const buildPreviewAndGo = async () => {
    ...
    if (hasLowQualityContent(openText)) { setError("Ten opis wygląda na zbyt przypadkowy albo testowy. Podaj minimum 2–3 konkretne zdania o sytuacji, żeby wynik miał sens."); return; }
    setBusy(true); setError(null);
    try {
      ...
      setStage("preview");
    } catch (e: any) { setError(e?.message || "Nie udało się przygotować podglądu."); setStage("error"); }
    finally { setBusy(false); }
  };
        ...
        <AnimatePresence mode="wait">
                ...
                <div className="hero-side-stack">
                  <Glass className="glass-panel story-panel visual-story">
                    ...
                    <div className="story-points">
                      ...
                      <div><span>▸</span><p>To, czego nie chcesz widzieć — bo tylko to może coś realnie zmienić</p></div>
                    </div>
                    <div className="story-lock">
                      <div className="story-lock-icon">🔒</div>
                      <div><strong>Pełny raport — 29 zł</strong><span>Jednorazowo. Raport przygotowany indywidualnie na podstawie Twoich odpowiedzi.</span></div>
                    </div>
                  </Glass>
                </div>
              </section>
                ...
                <Glass style={{ padding: "36px 40px" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "20px 40px" }}>
                    {[
                      ...
                      ["Nie jest mechaniczną ankietą.", "Pytania schodzą głębiej z każdą odpowiedzią — budują się na tym, co mówisz. To nie jest formularz do odhaczenia."]
                    ].map(([title, desc]) => (
          ...
          {stage === "consent" && (
            <motion.div key="consent" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <Glass className="question-panel consent-panel">
                ...
                <p className="consent-copy">To narzędzie nie pociesza. Nie zaokrągla. Nie daje Ci tego, co chcesz usłyszeć — daje Ci to, co wynika z Twoich odpowiedzi. Jeśli szukasz potwierdzenia że wszystko jest okej — to nie jest właściwe miejsce. Jeśli chcesz wiedzieć co naprawdę się dzieje — jesteś dokładnie tam gdzie trzeba.</p>
            ...
            <motion.div key={currentQuestion.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <div className="section-head compact">
                ...
                <div className="progress-wrap">
                  <span>Etap {questionIndex < 3 ? "1" : questionIndex < 6 ? "2" : "3"} z 3 — {questionIndex < 3 ? "obraz sytuacji" : questionIndex < 6 ? "mechanizm" : "lustro"}</span>
              ...
              <Glass className="question-panel">
                ...
                <div className="answer-grid">
                  {currentQuestion.options.map((opt, idx) => (
                    <button key={opt.id} className="answer-card" onClick={() => answerQuestion(currentQuestion.id, opt.id)}><span className="answer-letter">{String.fromCharCode(65 + idx)}</span><span>{opt.label}</span></button>
                  ))}
                </div>
          ...
          {stage === "interview" && interviewState && (
            <motion.div key={`interview-${interviewState.exchangeIndex}`} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              <Glass className="question-panel">
                ...
                <textarea className="ctms-textarea" value={interviewAnswer} onChange={(e) => { setInterviewAnswer(e.target.value); if (error) setError(null); }} placeholder="Odpowiedz konkretnie — co się dzieje, nie jak to interpretujesz. Im bardziej precyzyjnie, tym głębiej idzie analiza." maxLength={2000} />
                ...
                <div className="section-actions">
                  <GhostButton onClick={goBack}>Wróć</GhostButton>
                  <PrimaryButton onClick={sendInterviewAnswer} disabled={interviewBusy || interviewAnswer.trim().length < 10}>{interviewBusy ? "Sprawdzam..." : "Dalej →"}</PrimaryButton>
                </div>
              </Glass>
            </motion.div>
          )}
          ...
          {stage === "open_text" && path && (
            <motion.div key={`${path.key}-open`} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <Glass className="question-panel">
                ...
                <div className="question-copy">
                  <h3>{interviewState?.finished ? "Jest coś czego jeszcze nie powiedziałeś — a co może tu być kluczowe?" : path.openPrompt}</h3>
                ...
                <textarea className="ctms-textarea" value={openText} onChange={(e) => { setOpenText(e.target.value); if (error) setError(null); }} placeholder="Co się naprawdę dzieje — nie jak to interpretujesz, ale co konkretnie robisz i co ta osoba robi." maxLength={3000} />
                {error && <div className="error-line">{error}</div>}
                <div className="text-meta"><div>To jest rdzeń analizy.</div><div>{openText.length}/3000</div></div>
                <div className="section-actions">
                  <GhostButton onClick={goBack}>Wróć</GhostButton>
                  <PrimaryButton onClick={buildPreviewAndGo} disabled={busy || openText.trim().length < 10}>{busy ? "Sprawdzam..." : "Pokaż bezpłatny podgląd"}</PrimaryButton>
                </div>
              </Glass>
            </motion.div>
          )}
          ...
          {stage === "crisis" && (
            <motion.div key="crisis" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <Glass className="question-panel crisis-panel">
                ...
                <p className="consent-copy">Jeśli jest ryzyko przemocy, zagrożenia życia albo skrzywdzenia siebie — przerwij tę analizę i skorzystaj z natychmiastowej pomocy.</p>
            ...
            <motion.div key="preview" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <Glass className="preview-card">
                <div className="preview-hero">
                  <div className="eyebrow">BEZPŁATNY PODGLĄD</div>
                  <div className="pattern-chip">Wzorzec: {getPatternName(selectedPath, preview)}</div>
                ...
                <div className="relation-map">
                  {getRelationAxes(preview).map((axis) => (
                    <Glass key={axis.label} className="relation-axis">
                      <div className="relation-axis-top"><span>{axis.label}</span><strong>{axis.value}%</strong></div>
                      <div className="relation-axis-track"><div style={{ width: `${axis.value}%` }} /></div>
                      <p>{axis.hint}</p>
                    </Glass>
                  ))}
                </div>
                <Glass className="remember-line"><div className="eyebrow">ZDANIE DO ZAPAMIĘTANIA</div><p>{getRememberLine(selectedPath, preview)}</p></Glass>
                <div className="preview-grid">
                  <Glass className="report-section"><div className="eyebrow">CO JUŻ WIDAĆ</div><p>{preview.summary}</p></Glass>
                  <Glass className="report-section"><div className="eyebrow">MECHANIZM</div><p>{preview.tone === "green" ? "Najmocniej działa tu jeszcze wzajemność i struktura. To dobry znak, ale nie zwalnia z patrzenia na słabsze miejsca." : preview.tone === "yellow" ? "Napięcie miesza się tu z nadzieją i przywiązaniem. To nie przekreśla relacji, ale wymaga oddzielenia faktów od oczekiwań." : "Najmocniej pracuje tu napięcie, lęk przed stratą i potrzeba domknięcia historii. To nie znaczy automatycznie, że wszystko jest stracone — ale bez zmiany faktów sam sentyment nie wystarczy."}</p></Glass>
                ...
                <Glass className="unlock-panel">
                  ...
                  <p className="unlock-copy">Pełny raport pokazuje znacznie więcej niż bezpłatny podgląd: nazwę wzorca, mapę relacji, punkty ryzyka, miejsca możliwej zmiany i scenariusze dalszego rozwoju. Podgląd daje lustro. Pełny raport pokazuje całą strukturę sytuacji.</p>
                  <div className="unlock-form">
                    ...
                    <div className="unlock-note">Jednorazowa płatność. Dostęp na zawsze przez link w e-mailu. Raport jest zwykle gotowy w ciągu 1–2 minut.</div>
                  </div>
                </Glass>
                {error && <div className="error-line">{error}</div>}
                <div className="section-actions"><GhostButton onClick={goBack}>Wróć</GhostButton><GhostButton onClick={resetAll}>Od początku</GhostButton></div>
          ...
          {stage === "error" && (
            <motion.div key="error" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <Glass className="question-panel">
                <div className="eyebrow danger">COŚ POSZŁO NIE TAK</div>
                <h2>Coś poszło nie tak. Spróbuj wrócić krok wcześniej.</h2>
