...
function buildReportHtml(report: FullReport): string {
  const sections = (report.sections || [])
    .map((section, index) => `
      <section class="section ${escapeHtml(section.tone || "normal")}">
...
<head>
<meta charset="utf-8" />
<title>CzyToMaSens — pełny raport relacji</title>
<style>
  body{margin:0;background:#0d0c0b;color:#f5f1ea;font-family:Georgia,'Times New Roman',serif;line-height:1.65;padding:48px;}
  .wrap{max-width:980px;margin:0 auto;}
  .brand{color:#d7b978;letter-spacing:4px;font:700 11px Arial,sans-serif;text-transform:uppercase;margin-bottom:28px;}
  h1{font-size:42px;line-height:1.05;margin:0 0 14px;}
  .lead{font:18px Arial,sans-serif;color:#d7d0c7;margin-bottom:28px;}
  .metrics{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin:28px 0 36px;}
  .metric{border:1px solid rgba(215,185,120,.28);border-radius:18px;padding:18px;background:#161412;}
  .metric strong{display:block;font-size:30px;color:#d7b978;}
  .metric span{font:700 10px Arial,sans-serif;letter-spacing:3px;color:#a8a099;text-transform:uppercase;}
  .section{border:1px solid rgba(255,255,255,.12);border-left:4px solid #c5a059;border-radius:20px;padding:26px 28px;margin:18px 0;background:#151413;page-break-inside:avoid;}
  .section.danger{border-left-color:#e2a5a5}.section.gold{border-left-color:#d7b978}
  .num{font:700 11px Arial,sans-serif;letter-spacing:3px;color:#d7b978;margin-bottom:8px;}
  h2{font-size:24px;margin:0 0 14px;}
  p{font:17px Arial,sans-serif;color:#e8e0d8;margin:0 0 12px;}
  .closing{margin-top:30px;padding-top:20px;border-top:1px solid rgba(255,255,255,.12);font:18px Arial,sans-serif;color:#d7d0c7;}
  @media print{body{background:#fff;color:#111}.section{background:#fff;border-color:#ddd}.lead,p,.closing{color:#222}.brand,.metric strong,.num{color:#8a6b2e}}
</style>
</head>
<body><div class="wrap">
  <div class="brand">CzyToMaSens — prywatna analiza relacji</div>
  <h1>${escapeHtml(report.headline || "Pełny raport relacji")}</h1>
  <div class="lead">${escapeHtml(report.subheadline || "")}</div>
  <div class="metrics">
    <div class="metric"><strong>${Math.round(report.rebuildPercent || 0)}%</strong><span>realność zmiany</span></div>
    <div class="metric"><strong>${Math.round(report.tensionPercent || 0)}%</strong><span>napięcie</span></div>
    <div class="metric"><strong>${Math.round(report.driftPercent || 0)}%</strong><span>asymetria</span></div>
  </div>
  ${sections}
  <div class="closing">${escapeHtml(report.closing || "")}</div>
</div></body></html>`;
}
...
export default function App() {
  ...
  const downloadPaidReport = () => {
    ...
    a.href = url;
    a.download = "czytomasens-pelny-raport.html";
          ...
          {stage === "paid" && fullReport && (
            <motion.div key="paid" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <Glass className="preview-card premium-report-card">
                <div className="premium-report-toolbar no-print">
                  <div>
                    <div className="eyebrow">PEŁNY ODCZYT RELACJI</div>
                    <span className="premium-report-note">Raport wygenerowany indywidualnie na podstawie Twoich odpowiedzi.</span>
                  </div>
                  <div className="premium-actions">
                    <GhostButton onClick={printPaidReport}>Drukuj / zapisz PDF</GhostButton>
                    <PrimaryButton onClick={downloadPaidReport}>Pobierz raport</PrimaryButton>
                  </div>
                </div>

                <div className="premium-report-hero">
                  <div className="eyebrow">NAJWAŻNIEJSZY ODCZYT</div>
                  <h2>{fullReport.headline || "Ta relacja wymaga spokojniejszego spojrzenia niż kolejna rozmowa w napięciu."}</h2>
                  <p>{fullReport.subheadline || "W odpowiedziach widać zarówno miejsca, na których można budować, jak i punkty, których nie warto dalej tłumaczyć samą nadzieją."}</p>
                  {fullReport.previewLine && <div className="premium-report-line">{fullReport.previewLine}</div>}
                </div>

                {typeof fullReport.rebuildPercent === "number" && (
                  <div className="premium-metrics-strip">
                    {([[fullReport.rebuildPercent, "REALNOŚĆ ZMIANY", "czy za słowami widać powtarzalny ruch"], [fullReport.tensionPercent || 0, "NAPIĘCIE", "ile kosztuje Cię utrzymywanie tej sytuacji"], [fullReport.driftPercent || 0, "ASYMETRIA", "czy ciężar i jasność rozkładają się równo"]] as [number, string, string][]).map(([value, label, desc]) => (
                      <Glass key={label} className="premium-metric-card">
                ...
                <div className="report-sections premium-report-sections">
                  {(fullReport.sections || []).map((section, i) => (
                    <Glass key={i} className={`report-section premium-section report-section--${section.tone || "normal"}`}>
                ...
                <div className="section-actions no-print"><GhostButton onClick={resetAll}>Nowa analiza</GhostButton></div>
              </Glass>
            </motion.div>
          )}
