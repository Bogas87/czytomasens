...
function buildCycleSteps(pathKey?: EntryKey, burdens: BurdenItem[] = [], truthCards: string[] = []): string[] {
  ...
  return ["pierwszy sygnał", "miejsce kosztu", "konkret z życia", "wniosek dla Ciebie"];
}
...
function cycleStepDescription(step: string): string {
  ...
  const descriptions: Record<string, string> = {
    ...
    "działa": "są elementy, które nadal dają oparcie",
    "obciąża": "pojawia się koszt, którego nie warto pomijać",
    "wymaga sprawdzenia": "potrzebny jest przykład z życia, nie sama etykieta",
    "wniosek": "dopiero zachowanie pokaże, czy to zmiana",
    "co jeszcze działa": "co w tej relacji nadal ma grunt",
    "co już kosztuje": "gdzie pojawia się napięcie albo zmęczenie",
    "co trzeba sprawdzić": "jaki fakt z życia zmieniłby odczyt",
    "wniosek z faktów": "co można powiedzieć bez dopowiadania historii",
    ...
    "pierwszy sygnał": "pierwsza rzecz, która wybija się z odpowiedzi",
    "miejsce kosztu": "gdzie relacja zaczyna zabierać spokój",
    "konkret z życia": "jedna sytuacja, która może zmienić ocenę",
    "wniosek dla ciebie": "co z tego wynika w praktyce",
...
function buildAdaptiveCycle(mode: InsightMode, variant: PauseVariant, pathKey?: EntryKey, burdens: BurdenItem[] = [], truthCards: string[] = []): string[] {
  ...
  if (mode === "mixed") {
    ...
    return ["co jeszcze działa", "co już kosztuje", "co trzeba sprawdzić", "wniosek z faktów"];
  }
    ...
    <motion.div key={`${insight.eyebrow}-${insight.title}`} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
      ...
      <Glass className={`question-panel relationship-map-panel pause-panel ${insight.mode}`}>
        ...
        <div className="pause-grid">
          ...
          <div className="pause-visual-card">
            <div className="eyebrow">CO SPRAWDZAMY DALEJ</div>
        ...
        <AnimatePresence mode="wait">
          ...
          {stage === "consent" && (
            <motion.div key="consent" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <Glass className="question-panel consent-panel">
                ...
                <p className="consent-copy">To narzędzie nie ma Cię straszyć ani pocieszać. Ma uporządkować fakty, reakcje i powtarzające się momenty tak, żeby wynik miał sens dla tej jednej relacji, a nie dla ogólnej teorii o związkach.</p>
          ...
          {stage === "questions" && path && currentQuestion && (
            <motion.div key={currentQuestion.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              <Glass className="question-panel question-panel--single">
                <div className="single-question-label"><span>Pytanie {questionIndex + 1}</span><em>z {path.questions.length}</em></div>
            ...
            <motion.div key={`${path.key}-force-map`} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              <Glass className="question-panel relationship-map-panel">
                <div className="map-step-note">
                  Każda karta to osobne pytanie. Zaznacz najbliższą odpowiedź. Nie musi pasować idealnie.
                </div>
          ...
          {stage === "interview" && interviewState && (
            <motion.div key={`interview-${interviewState.exchangeIndex}`} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              <Glass className="question-panel question-panel--single">
                <div className="single-question-label">Pytanie pogłębiające</div>
                <div className="question-copy">
